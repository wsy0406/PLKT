...
class MarginBCELoss(nn.Module):
    ...
    def forward(self, pos_score, neg_score, label):
        """
        Args:
            pos_score: (B, 1) 答对 logit
            neg_score: (B, 1) 答错 logit
            label: (B, 1) 学生真实答题结果 (1=答对, 0=答错)
        """
        logits = pos_score - neg_score  # (B, 1)
        y_pred = torch.sigmoid(logits)

        bce_loss = F.binary_cross_entropy(y_pred, label.float(), reduction="none")


        pos_margin_violation = torch.relu(self.margin - logits) * label
        neg_margin_violation = torch.relu(self.margin + logits) * (1 - label)
        margin_loss = pos_margin_violation + neg_margin_violation
...
def cal_loss(model, ys, r, rshft, sm, preloss=[]):
    ...
    elif model_name in ["rkt", "dimkt", "dkt", "dkt_forget", "dkvmn", "deep_irt", "kqn", "saint","sakt", "atkt",
                        "atktfix", "gkt", "skvmn", "hawkes", "vkt"]:
        y = torch.masked_select(ys[0], sm)
        t = torch.masked_select(rshft, sm)
        loss = binary_cross_entropy(y.double(), t.double())

    elif model_name in ["ptkt"]:
        y = ys[0]
        t = rshft[:,-1].unsqueeze(-1)
        loss = binary_cross_entropy(y.double(), t.double())
       
    elif model_name in ['autoptkt']:
        y = ys[0]
        t = rshft[:, -1].unsqueeze(-1)
        kt_loss = binary_cross_entropy(y.double(), t.double())
        reg_loss = torch.norm(model.selector, p=2)
        div_loss = diversity_loss(model)
        loss = kt_loss + model.args.reg_lambda * reg_loss + model.args.diversity_lambda * div_loss
...
def diversity_loss(model):
    """
        selector: [N, S]
    """
    selector = F.normalize(model.selector, dim=-1)
    cos_sim = torch.mm(selector, selector.T)  # [N, N]
    mask = torch.eye(cos_sim.shape[0]).to(model.selector.device)  # [N, N]
    cos_sim = cos_sim.masked_fill(mask.bool(), 0)  # [N, N]
    diversity_loss = cos_sim.abs().mean()
    return diversity_loss

def model_forward(model, data, device, tau=None, rel=None):
    model_name = model.model_name
    new = None
    ...
    elif model_name in ["ptkt","autoptkt"]:
        q, c, r = dcur["new_qseqs"].to(device), dcur["new_cseqs"].to(device), dcur["new_rseqs"].to(device)
        qshft, cshft, rshft = dcur["shft_new_qseqs"].to(device), dcur["shft_new_cseqs"].to(device), dcur[
            "shft_new_rseqs"].to(device)
        m, sm = dcur["new_masks"].to(device), dcur["new_smasks"].to(device)
        new = 1
    ...
    if model_name in ["ptkt","autoptkt"]:
        cq = torch.cat((q, qshft[:,-1].unsqueeze(-1)), dim=1)
        cc = torch.cat((c, cshft[:,-1].unsqueeze(-1)), dim=1)
        cr = torch.cat((r, rshft[:, -1].unsqueeze(-1)), dim=1)
    ...
    if model_name in ["dkt"]:
        ...
        y = model(q.long(), c.long(), r.long(), qshft.long(), cshft.long(), m)
        ys.append(y)
    elif model_name in ["autoptkt"]:
        y = model(q.long(), c.long(), r.long(), qshft.long(), cshft.long(), m, tau)
        ys.append(y)

    elif model_name in ['vkt']:
        y = model(c.long(), r.long(), cshft.long())
        y = torch.sigmoid(y)
        ys.append(y)
...
def train_model(model, train_loader, valid_loader, num_epochs, opt, ckpt_path, device, args, test_loader=None,
                test_window_loader=None, save_model=False, data_config=None, fold=None, writer=None):
    ...
    for i in range(1, num_epochs + 1):
        ...
        for data in train_loader:
            ...
            elif model.model_name == 'autoptkt':
                tau = model.temperature_schedule(i, args.anneal_epoch, start_temp=1.0, end_temp=args.min_tau)
                loss, new = model_forward(model, data, device, tau)
            elif model.model_name in ['ptkt']:
                # loss, new, pattern_weight_batch = model_forward(model, data, device)
                loss, new = model_forward(model, data, device)
                # pattern_weight_epoch.append(pattern_weight_batch.detach().cpu().tolist())
            else:
                loss, new = model_forward(model, data, device)
            opt.zero_grad()
        ...
        if model.model_name == 'rkt':
            ...
            auc, acc = evaluate(model, valid_loader, model.model_name, device, new=1)
        elif model.model_name == 'ptkt':
            auc, acc = evaluate(model, valid_loader, model.model_name, device, new=1)
        else:
            print("++++++++++++++++++++++++++++++++++++++"+str(new)+"+++++++++++++++++++++++++++++++++++++")
            auc, acc = evaluate(model, valid_loader, model.model_name, device)
        writer.add_scalar('train/auc', auc, i)
        ...
        if auc > max_auc + 1e-3:
            ...
            if not save_model:
                if test_loader != None:
                    ...
                    testauc, testacc = evaluate(model, test_loader, model.model_name, device, None, save_test_path)
                if test_window_loader != None:
                    save_test_path = os.path.join(ckpt_path, model.emb_type + "_test_window_predictions.txt")
                    window_testauc, window_testacc = evaluate(model, test_window_loader, model.model_name, device, None,
                                                              save_test_path)
