...
def evaluate(model, test_loader, model_name, device, new=None, rel=None, save_path=""):
    if save_path != "":
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if model_name in ["dimkt"]:
                ...
                if new != None:
                    q, c, r = dcur["new_qseqs"], dcur["new_cseqs"], dcur["new_rseqs"]
                    qshft, cshft, rshft = dcur["shft_new_qseqs"], dcur["shft_new_cseqs"], dcur["shft_new_rseqs"]
                else:
                    q, c, r = dcur["qseqs"], dcur["cseqs"], dcur["rseqs"]
                    qshft, cshft, rshft= dcur["shft_qseqs"], dcur["shft_cseqs"], dcur["shft_rseqs"]
            if new != None:
                m, sm = dcur["new_masks"], dcur["new_smasks"]
            else:
            ...
            if model_name in ["ptkt","autoptkt"]:
                # indices = (sm.sum(dim=-1, keepdim=True) - 1).long()  # (B, 1)
                y = y.detach().cpu()
                t = rshft[:,-1].squeeze(-1).detach().cpu()
                # t = torch.gather(rshft, dim=-1, index=indices).detach().cpu()
